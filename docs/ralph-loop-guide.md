...
**When**: Server/API project types (express, nextjs, flask, python, reflex, go, actix, rocket, rails, ruby, dotnet webapi/mvc/blazor) OR when the PRD contains API keywords (REST, endpoint, route, POST, GET, CRUD, HTTP, middleware, authentication). Skipped for frontend-only types (react, angular, blazorwasm, nx).

**Approach**: In-process HTTP clients — no live server required, hermetic, fast.

**Tools by stack**:
- Express/Next.js: `supertest`
- Flask: `app.test_client()` (built-in)
- Python/FastAPI: `httpx` + `pytest-asyncio`
- Go: `net/http/httptest` (stdlib)
- Rails: RSpec request specs
- Rust (actix/rocket): built-in test modules
- .NET WebAPI/MVC: `WebApplicationFactory`

**Example Structure**:
```
tests/integration/
├── test_req_001_integration.py   # pytest / Flask / httpx
├── req-001.integration.test.ts   # jest / supertest
└── req_001_integration_test.go   # Go httptest
```
